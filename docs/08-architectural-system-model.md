...
从架构角度看，目标系统 更适合被定义为一套 **terminal-native agent runtime platform**，而不是一个“命令行工具集合”或“对话界面叠加若干工具”的实现。
...
从职责而不是目录命名出发，目标系统 可以划分为 7 个一级子系统。
...
从系统结构上看，目标系统 的核心并不在终端 UI，而在以下四个平面之间的协作关系：
...
目标系统 不严格对应某一种经典架构模板，但其组织方式呈现出以下几种风格的组合特征：
