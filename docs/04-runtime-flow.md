...
`main.tsx` 不是一个简单的 CLI 入口，而是 Claude Code 的**启动编排器**。
...
这说明 Claude Code 很在意启动时延，很多 I/O 在 import 阶段就并行发起了。

### 第 2 步：CLI 参数解析
`main.tsx` 里大量逻辑围绕 commander / flags / feature gate 组合。

说明 Claude Code 不是“只有一个 `claude` 命令然后全靠自然语言”，而是一个功能非常重的 CLI 产品。
...
`query.ts` 是 Claude Code 交互路径中的**主 query loop**。
...
### 为什么它重要
Claude Code 的“智能行为”不在单次模型调用里，而在这个 loop 里。
...
### 它的价值
这个文件说明 Claude Code 在产品内部已经把“会话引擎”和“终端 UI”做了一定解耦。
...
这说明 Claude Code 不是“等整条 assistant 完成再执行所有工具”，而是已经在尝试更强的实时代理体验。
...
这是 Claude Code 生命周期 hooks 的大本营。
...
Stop 阶段是 Claude Code 里很重要的一环，因为一轮 agent 行为不只是“assistant 说完话就结束”。
...
### 架构意义
这说明 Claude Code 把“回合结束”本身视作一个独立的生命周期阶段，而不是 query loop 的尾巴。
...
这是 Claude Code 里最像“多代理 runtime”的文件之一。
...
Claude Code 则是把 `AgentTool` 做成一个正式工具，把子代理生命周期、输出、路由、任务化都建模了。
...
### 结论
skills 是 Claude Code 的正式扩展层，不只是提示词片段。

---

## 10. MCP 主线：`services/mcp/client.ts`

Claude Code 的 MCP 客户端不是薄封装，而是一个很重的子系统。
...
Claude Code 的核心并不是某个文件，而是几条主线的协作：
